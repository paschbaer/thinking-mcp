...
- [x] T005 [US1] Implement `runOperation(sessionId, operationId)` on `WorkflowEngine` (reuse lifecycle path: OperationEngine.execute + exposeOpResult + audit; per-session mutex per FR-107; workspace-level lock file for venv-mutating ops per FR-109; kill child process + release locks on session cancel/timeout per FR-110) and register the `run_operation` tool in `src/mcp-server/register-tools.ts` + handler — make T004 pass
...
- [x] T010 [US2] Run full regression suite + typecheck + build (SC-005); fix fallout; verify no npm-path regressions in default profile (lint/test/build ops unchanged)
