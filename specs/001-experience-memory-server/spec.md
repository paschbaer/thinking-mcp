...
- **FR-008a**: Fabricated-evidence mitigation boundary for the MVP: the system mitigates fabricated evidence via content-addressed artifact storage (hash-verified on read, FR-031) and pattern-based redaction (FR-025) — a validation run's evidence artifact must exist on disk with a matching hash at finalize time. Cryptographic attestation of the execution environment (who actually ran the test — sigstore/in-toto style attestations) is explicitly OUT OF SCOPE for the MVP and deferred to the team phase. Acceptance criterion for the MVP: an artifact file that is deleted or modified between `validation.record_run` and `experience.finalize` MUST cause finalize to fail with `ARTIFACT_HASH_MISMATCH` (or `MISSING_REQUIRED_EVIDENCE`), never to verify silently.
