...
export const WORKFLOW_TOOL_NAMES = [
  "start_workflow", "get_current_guidance",
  ...
  "complete_workflow", "get_workflow_state",
  "report_blocker", "resume_workflow", "cancel_workflow",
  "get_orchestration_status", "list_configured_operations", "retry_operation", "get_downstream_status",
] as const;
...
export function registerWorkflowTools(server: McpServer, tools: WorkflowTools, workspaceRoot: string): void {
  ...
  server.tool(
    ...
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitUnderstanding(sessionId, payload, requestId)),
  );
  server.tool(
    "submit_plan",
    "Reicht den Implementierungsplan ein",
    { ...sessionId, ...requestId, tasks: z.array(z.record(z.unknown())) },
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitPlan(sessionId, payload, requestId)),
  );
  server.tool(
    "submit_plan_review",
    ...
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitPlanReview(sessionId, payload, requestId)),
  );
  server.tool(
    ...
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitImplementation(sessionId, payload, requestId)),
  );
  server.tool(
    ...
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitImplementationReview(sessionId, payload, requestId)),
  );
  server.tool(
    "submit_verification",
    "Reicht Verifizierungsergebnisse ein",
    { ...sessionId, ...requestId, verificationSummary: z.array(z.string()) },
    async ({ sessionId, requestId, ...payload }) => toJson(await tools.submitVerification(sessionId, payload, requestId)),
  );
  server.tool(
    "complete_workflow",
    "Reicht den Abschlussbericht ein und fordert Completion an",
    { ...sessionId, ...requestId, summary: z.string() },
    async ({ sessionId, requestId, summary }) => toJson(await tools.completeWorkflow(sessionId, { summary }, requestId)),
  );
  server.tool(
    "get_workflow_state",
    "Liest den persistierten Session-Zustand",
    { ...sessionId, includeHistory: z.boolean().optional() },
    async ({ sessionId }) => ({ content: [{ type: "text" as const, text: JSON.stringify(await tools.getWorkflowState(sessionId)) }] }),
  );
  server.tool(
    "report_blocker",
    ...
    async ({ sessionId, ...rest }) => toJson(await tools.reportBlocker(sessionId, rest)),
  );
  server.tool(
    "resume_workflow",
    "Beendet 'blocked' und kehrt in die vorherige Phase zurück",
    { ...sessionId, decision: z.string(), notes: z.string().optional() },
    async ({ sessionId, decision, notes }) => toJson(await tools.resumeWorkflow(sessionId, { decision, notes })),
  );
  server.tool(
    "cancel_workflow",
  ...
  server.tool(
    ...
    sessionId,
    async ({ sessionId }) => ({ content: [{ type: "text" as const, text: JSON.stringify(await tools.getOrchestrationStatus(sessionId)) }] }),
  );
  server.tool(
    "list_configured_operations",
    "Sichere Liste konfigurierter Operationen",
    {},
    async () => ({ content: [{ type: "text" as const, text: JSON.stringify(tools.listConfiguredOperations()) }] }),
  );
  server.tool(
    "retry_operation",
  ...
  server.tool(
    ...
    {},
    async () => ({ content: [{ type: "text" as const, text: JSON.stringify(await tools.getDownstreamStatus()) }] }),
  );
}
