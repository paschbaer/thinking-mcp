    ...
    specKit?: {
      ...
      taskExecution?: {
        ...
        batch?: {
          ...
          groupParallelTasks?: boolean;
        };
      };
      markdownCheckboxPolicy?:
        | "ignore"
        | "hint"
        | "implemented_claim"
        | "completed_only_with_evidence";
      changes?: {
        allowAutomaticMinorAdjustments?: boolean;
        requireArtifactUpdateFor?: string[];
      };
...
export interface SpecKitConfig {
  ...
  markdownCheckboxPolicy:
    "ignore" | "hint" | "implemented_claim" | "completed_only_with_evidence";
  changes: {
    allowAutomaticMinorAdjustments: boolean;
    requireArtifactUpdateFor: string[];
  };
...
function getValidateMain(): ValidateMain {
  if (!cachedValidateMain) {
    const Ajv2020Ctor = Ajv2020 as unknown as new (opts: object) => {
      compile: (s: unknown) => ValidateMain;
    };
...
function readJsonFile(path: string, label: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new ConfigurationError(
      "configuration_invalid",
      `${label} is not valid JSON: ${String(err)}`,
    );
  }
}
...
function isSafeRelative(rel: string): boolean {
  ...
  return (
    !segments.includes("..") &&
    segments.every((seg) => seg.length > 0 && seg !== ".")
  );
}
...
function buildSpecKitConfig(main: GuidanceMainConfig): SpecKitConfig {
  ...
  if (!isSafeRelative(root)) {
    throw new ConfigurationError(
      "configuration_invalid",
      `specKit discovery.featureRoot escapes the workspace: ${root}`,
    );
  }
  return {
    discovery: {
    ...
    taskExecution: {
      ...
      batch: {
        ...
        respectDependencies:
          sk.taskExecution?.batch?.respectDependencies ?? true,
        groupParallelTasks: sk.taskExecution?.batch?.groupParallelTasks ?? true,
      },
    },
    markdownCheckboxPolicy: sk.markdownCheckboxPolicy ?? "hint",
    changes: {
      allowAutomaticMinorAdjustments:
        sk.changes?.allowAutomaticMinorAdjustments ?? false,
...
function validateDownstreamServers(data: Record<string, unknown>): void {
  ...
  if (typeof servers !== "object" || Array.isArray(servers)) {
    throw new ConfigurationError(
      "configuration_invalid",
      "downstreamServers.servers must be an object",
    );
  }
  for (const [id, raw] of Object.entries(
    ...
    for (const key of [
      ...
      if (
        ...
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: connection.${key} must be a positive finite number`,
        );
      }
    }
    ...
    if (reconnect !== undefined) {
      if (typeof reconnect !== "object" || Array.isArray(reconnect)) {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: connection.reconnect must be an object`,
        );
      }
      const rc = reconnect as {
        enabled?: unknown;
        maximumAttempts?: unknown;
        delayMilliseconds?: unknown;
      };
      if (rc.enabled !== undefined && typeof rc.enabled !== "boolean") {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: connection.reconnect.enabled must be a boolean`,
        );
      }
      if (
        rc.maximumAttempts !== undefined &&
        (typeof rc.maximumAttempts !== "number" ||
          !Number.isInteger(rc.maximumAttempts) ||
          rc.maximumAttempts < 1)
      ) {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: connection.reconnect.maximumAttempts must be a positive integer`,
        );
      }
      if (
        rc.delayMilliseconds !== undefined &&
        (typeof rc.delayMilliseconds !== "number" ||
          !Number.isInteger(rc.delayMilliseconds) ||
          rc.delayMilliseconds < 0)
      ) {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: connection.reconnect.delayMilliseconds must be a non-negative integer`,
        );
      }
    }
    ...
    if (typeof transport !== "object" || Array.isArray(transport)) {
      throw new ConfigurationError(
        "configuration_invalid",
        `downstreamServers.${id}: transport must be an object`,
      );
    }
    const tr = transport as {
      type?: unknown;
      command?: unknown;
      http?: unknown;
    };
    if (
      tr.type !== undefined &&
      !TRANSPORT_TYPES.includes(tr.type as (typeof TRANSPORT_TYPES)[number])
    ) {
      throw new ConfigurationError(
        "configuration_invalid",
        `downstreamServers.${id}: transport.type must be one of ${TRANSPORT_TYPES.join(" | ")}`,
      );
    }
    if ((tr.type ?? "stdio") === "stdio") {
      const command = tr.command as
        { executable?: unknown; args?: unknown } | undefined;
      if (
        ...
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: transport.command.executable is required for stdio transport`,
        );
      }
      if (
        ...
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: transport.command.args must be an array of strings`,
        );
      }
    } else {
      ...
      if (typeof http?.url !== "string" || http.url.length === 0) {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: transport.http.url is required for http transport`,
        );
      }
      let parsed: URL;
      try {
        ...
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: transport.http.url is not a valid URL`,
        );
      }
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        throw new ConfigurationError(
          "configuration_invalid",
          `downstreamServers.${id}: transport.http.url must use http(s)`,
        );
      }
      if (http.headers !== undefined) {
        if (typeof http.headers !== "object" || Array.isArray(http.headers)) {
          throw new ConfigurationError(
            "configuration_invalid",
            `downstreamServers.${id}: transport.http.headers must be an object of strings`,
          );
        }
        for (const [k, v] of Object.entries(
          http.headers as Record<string, unknown>,
        )) {
          if (typeof v !== "string") {
            throw new ConfigurationError(
              "configuration_invalid",
              `downstreamServers.${id}: transport.http.headers.${k} must be a string`,
            );
          }
        }
      }
...
function applyHttpTransports(
  ...
  const policies = loaded["policies"] as
    { egress?: { httpHostAllowlist?: unknown } } | undefined;
  const allowlistRaw = policies?.egress?.httpHostAllowlist;
  if (
    !Array.isArray(allowlistRaw) ||
    allowlistRaw.some((e) => typeof e !== "string" || e.length === 0)
  ) {
    throw new ConfigurationError(
      "configuration_invalid",
...
function validateOperations(operationsFile: Record<string, unknown>): void {
  ...
  for (const [id, raw] of Object.entries(
    ops as Record<string, Record<string, unknown>>,
  )) {
    const type = raw["type"];
    if (typeof type !== "string" || !OPERATION_TYPES.includes(type)) {
      throw new ConfigurationError(
        "configuration_invalid",
        `operations.${id}: unknown type ${String(type)}`,
      );
    }
    ...
    if (env !== undefined) {
      if (typeof env !== "object" || env === null || Array.isArray(env)) {
        throw new ConfigurationError(
          "configuration_invalid",
          `operations.${id}: env must be an object of strings`,
        );
      }
      for (const [k, v] of Object.entries(env as Record<string, unknown>)) {
        if (typeof v !== "string") {
          throw new ConfigurationError(
            "configuration_invalid",
            `operations.${id}: env.${k} must be a string`,
          );
        }
      }
    }
    ...
    if (
      ...
      throw new ConfigurationError(
        "configuration_invalid",
        `operations.${id}: shell must be a boolean or a string`,
      );
    }
    const sampling = raw["sampling"] as
      | {
          purpose?: unknown;
    ...
    if (type === "sampling") {
      if (
        typeof sampling?.purpose !== "string" ||
        sampling.purpose.length === 0
      ) {
        throw new ConfigurationError(
          "configuration_invalid",
          `operations.${id}: sampling requires a purpose`,
        );
      }
      if (
        sampling.maxOutputTokens !== undefined &&
        (typeof sampling.maxOutputTokens !== "number" ||
          sampling.maxOutputTokens < 1)
      ) {
        throw new ConfigurationError(
          "configuration_invalid",
          `operations.${id}: sampling.maxOutputTokens must be a positive number`,
        );
      }
      if (
        sampling.maximumAttempts !== undefined &&
        (typeof sampling.maximumAttempts !== "number" ||
          sampling.maximumAttempts < 1)
      ) {
        throw new ConfigurationError(
          "configuration_invalid",
          `operations.${id}: sampling.maximumAttempts must be a positive number`,
        );
      }
    }
...
export function loadConfig(configDir: string): LoadedConfig {
  ...
  for (const key of [
    ...
    if (ref?.file) {
      ...
      if (!existsSync(path)) {
        throw new ConfigurationError(
          "configuration_invalid",
          `${key}: referenced file missing: ${ref.file}`,
        );
      }
  ...
  const chain: ChainConfig = {
    ...
    maxChainDepth: cfg.chain?.maxChainDepth ?? 8,
