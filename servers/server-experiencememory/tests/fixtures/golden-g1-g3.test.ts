...
it('T046 golden paths', async () => {
  ...
  const s1 = await call(client, 'workflow.start', { goal: 'clean install exits 0', scope_id: CTX.scope_id, problem_summary: 'ERESOLVE', idempotency_key: 'g1-1', client_context: CTX });
  ...
  const s2 = await call(client, 'experience.record_observation', { workflow_id: wf, kind: 'failure_output', content: 'npm ERR code ERESOLVE exited with code 1', exit_code: 1, expected_revision: 1, client_context: CTX });
  ...
  const s3 = await call(client, 'experience.record_observation', { workflow_id: wf, kind: 'environment_fact', content: JSON.stringify({ os: 'linux', node: '20' }), expected_revision: 2, client_context: CTX });
  record('G1.3 environment fact recorded, rev 3', (s3.result as Record<string, unknown>).new_revision === 3);

  const s4bad = await call(client, 'experience.record_attempt', { workflow_id: wf, intent: 'disable peer dependency checks', expected_revision: 3, client_context: CTX });
  const s4b = await call(client, 'experience.complete_attempt', { workflow_id: wf, attempt_id: (s4bad.result as Record<string, unknown>).attempt_id as string, outcome: 'runtime failures', classification: 'harmful', expected_revision: 4, client_context: CTX });
  record('G1.4a harmful attempt recorded (known-bad), rev 5', (s4b.result as Record<string, unknown>).new_revision === 5);

  const s4 = await call(client, 'experience.record_attempt', { workflow_id: wf, intent: 'align peer dependency versions', expected_revision: 5, client_context: CTX });
  ...
  const s5 = await call(client, 'experience.complete_attempt', { workflow_id: wf, attempt_id: attId, outcome: 'installed cleanly', classification: 'successful', expected_revision: 6, client_context: CTX });
  record('G1.5 attempt completed successful, rev 7', (s5.result as Record<string, unknown>).new_revision === 7);

  const s6 = await call(client, 'experience.propose_solution', {
  ...
  const s7 = await call(client, 'artifact.attach', { workflow_id: wf, content_base64: Buffer.from('install log: exit 0').toString('base64'), kind: 'test_report', media_type: 'text/plain', expected_revision: 8, client_context: CTX });
  ...
  const s8 = await call(client, 'validation.record_run', { workflow_id: wf, check_index: 0, status: 'passed', exit_code: 0, evidence_artifact_id: artifactId, expected_revision: 9, client_context: CTX });
  record('G1.8a original-failure check passed w/ evidence', !!(s8.result as Record<string, unknown>).run_id);

  const s9 = await call(client, 'validation.record_run', { workflow_id: wf, check_index: 1, status: 'passed', exit_code: 0, evidence_artifact_id: artifactId, expected_revision: 10, client_context: CTX });
  ...
  const s10 = await call(client, 'experience.finalize', { workflow_id: wf, requested_outcome: 'verified', expected_revision: 11, client_context: CTX });
  record('G1.9 finalize(verified) -> LOCALLY_VERIFIED', (s10.result as Record<string, unknown>).final_state === 'LOCALLY_VERIFIED');

  const s11 = await call(client, 'experience.search', { query: 'ERESOLVE dependency installation', scope_id: CTX.scope_id, environment: { os: 'linux', node: '20' } });
  ...
  const n1 = await call(client, 'workflow.start', { goal: 'neg', scope_id: CTX.scope_id, idempotency_key: 'g1-neg', client_context: CTX });
  const nwf = (n1.result as Record<string, unknown>).workflow_id as string;
  await call(client, 'experience.record_observation', { workflow_id: nwf, kind: 'failure_output', content: 'boom exited with code 1', exit_code: 1, expected_revision: 1, client_context: CTX });
  await call(client, 'experience.record_observation', { workflow_id: nwf, kind: 'environment_fact', content: JSON.stringify({ os: 'linux', node: '20' }), expected_revision: 2, client_context: CTX });
  const natt = await call(client, 'experience.record_attempt', { workflow_id: nwf, intent: 'retry same', expected_revision: 3, client_context: CTX });
  await call(client, 'experience.complete_attempt', { workflow_id: nwf, attempt_id: (natt.result as Record<string, unknown>).attempt_id as string, outcome: 'ok', classification: 'successful', expected_revision: 4, client_context: CTX });
  await call(client, 'experience.propose_solution', { workflow_id: nwf, strategy: 's', mechanism: 'm', checks: [
  ...
  const nev = await call(client, 'artifact.attach', { workflow_id: nwf, content_base64: Buffer.from('ok').toString('base64'), kind: 'log', media_type: 'text/plain', expected_revision: 6, client_context: CTX });
  await call(client, 'validation.record_run', { workflow_id: nwf, check_index: 0, status: 'passed', exit_code: 0, evidence_artifact_id: (nev.result as Record<string, unknown>).artifact_id as string, expected_revision: 7, client_context: CTX });
  const nfin = await call(client, 'experience.finalize', { workflow_id: nwf, requested_outcome: 'verified', expected_revision: 8, client_context: CTX });
  ...
  const g2 = await call(client, 'experience.search', { query: 'ERESOLVE dependency installation', scope_id: CTX.scope_id, environment: { os: 'linux', node: '22' } });
  ...
  const g3search = await call(client, 'experience.search', { query: 'ERESOLVE dependency installation', scope_id: CTX.scope_id, environment: { os: 'linux', node: '20' } });
  ...
  const g3fb = await call(client, 'experience.record_reuse_feedback', { workflow_id: g3wf, experience_id: g3top.experience_id, verdict: 'harmful', client_context: CTX });
  ...
  const s2nd = await call(client, 'workflow.start', { goal: 'g3 peer', scope_id: CTX.scope_id, idempotency_key: 'g3-peer', client_context: CTX });
  ...
  await call(client, 'experience.record_observation', { workflow_id: pwf, kind: 'failure_output', content: 'npm ERR code ERESOLVE exited with code 1', exit_code: 1, expected_revision: prev++, client_context: CTX });
  await call(client, 'experience.record_observation', { workflow_id: pwf, kind: 'environment_fact', content: 'node 20 linux', expected_revision: prev++, client_context: CTX });
  await call(client, 'experience.record_attempt', { workflow_id: pwf, intent: 'clean align', expected_revision: prev++, client_context: CTX });
  const peerEv = await call(client, 'artifact.attach', { workflow_id: pwf, content_base64: Buffer.from('pass').toString('base64'), kind: 'log', media_type: 'text/plain', expected_revision: prev++, client_context: CTX });
  await call(client, 'experience.propose_solution', { workflow_id: pwf, strategy: 'clean align', mechanism: 'semver', checks: [{ criterion: 'clean install exits 0', test_type: 'build', expected_result: 'exit 0', regression_coverage: false, timeout_s: 300, evidence_requirement: true, targets_original_failure: true }], expected_revision: prev++, client_context: CTX });
  await call(client, 'validation.record_run', { workflow_id: pwf, check_index: 0, status: 'passed', exit_code: 0, evidence_artifact_id: (peerEv.result as Record<string, unknown>).artifact_id as string, expected_revision: prev++, client_context: CTX });
  await call(client, 'experience.finalize', { workflow_id: pwf, requested_outcome: 'verified', expected_revision: prev, client_context: CTX });

  const after = await call(client, 'experience.search', { query: 'ERESOLVE dependency installation', scope_id: CTX.scope_id, environment: { os: 'linux', node: '20' } });
  ...
  // ===== Zusatzszenarien =====
  const i1 = await call(client, 'workflow.start', { goal: 'idem', scope_id: CTX.scope_id, idempotency_key: 'idem-1', client_context: CTX });
  const i2 = await call(client, 'workflow.start', { goal: 'idem', scope_id: CTX.scope_id, idempotency_key: 'idem-1', client_context: CTX });
  ...
  const st1 = await call(client, 'experience.record_observation', { workflow_id: nwf, kind: 'environment_fact', content: 'stale test', expected_revision: 99, client_context: CTX });
  ...
  const o1 = await call(client, 'experience.search', { query: 'ERESOLVE', scope_id: 'other-repo' });
