...
**experience-memory** MCP server (HTTP: `http://localhost:3002/mcp`,
registered user-scope as `insight`; single store of truth:
`servers/server-insight/emms-data/emms-store.db`):
...
- No hit: proceed normally — and if the session uncovers a new recurring trap,
  seed it via the `experience_seed_lessons` MCP tool (server-side, idempotent
  per slug; see `.github/prompts/capture-lessons.prompt.md` — the MASTER
  prompt). The thin client `servers/server-insight/scripts/seed-lessons.mjs`
