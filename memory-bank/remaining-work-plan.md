...
- [ ] CB-4 MEDIUM Deployment: Root-Compose veröffentlicht 3000+3002 auf 0.0.0.0; insight hat KEINE Auth (scope_id caller-supplied) → voller EMMS-Store-Zugriff aus dem LAN ohne Firewall. Trigger: Betrieb außerhalb localhost / LAN-Exposition. Action required: Auth (Pattern GUIDANCE_AUTH_TOKEN) oder 127.0.0.1-Port-Bind in compose.
- [ ] CB-5 LOW stochasticthinking: `app.listen(PORT)` ohne Host-Arg → default 0.0.0.0 (einziger Server ohne Loopback-Default), keine Auth, kein npm-deprecated-Marker trotz README-Deprecation. Trigger: falls Server weitergenutzt wird. Action: Host-Arg setzen oder `npm deprecate`.
- [ ] CB-6 LOW Root package.json: `publish:insight`/`npm:publish:insight` zeigen auf `servers/insight` (existiert nicht; korrekt `servers/server-insight`) → Scripts brechen. Trigger: nächster insight-Publish. Action required.
- [ ] CB-7 LOW `emms-store.db-wal` ist git-getrackt trotz `/emms-store.db-wal` in .gitignore (tracked-before-ignored; Binär-WAL im VCS). Trigger: nächstes Repo-Hygiene-Cleanup. Action: `git rm --cached`.
- [ ] CB-8 LOW Guidance server.ts HTTP-Pre-Checks (L137-168) nehmen einzelnes JSON-RPC-Objekt an; Array-Body überspringt Auth/Rate-Limit/Binding-Pre-Checks (SDK verwirft Batches — Defense-in-Depth). Trigger: nächste HTTP-Härtungsrunde. Accepted observation mit optionaler Härtung.
...
- [ ] CB-10 LOW Guidance `initSession` schreibt meta.json zweimal (L215+L222) — Crash-Fenster verliert canonicalHash (M2-Persistenz). Trigger: nächste Persistenzrunde. Accepted observation, Fix-Kandidat: ein Write.
...
- [ ] CB-13 LOW README: duplizierter Dev-Loop-Block (L115-119 ≡ L125-129) + engines-Drift (README >=20 vs root package.json >=18). Trigger: nächstes README-Update. Action required.
...
- [ ] CB-14 HIGH-Kandidat (entdeckt 2026-09-24, Batch B): Das committedete `yarn.lock` ist im Yarn-Classic-Format (v1, kein `__metadata`), während `packageManager` yarn@4.6.0 pinnt. Ein `corepack yarn install` (4.6.0) scheitert an der Migration („not present in your lockfile") bzw. rewiwrite das Lockfile massiv ohne --immutable. CI (`test.yml`: `corepack prepare yarn@4.6.0 && yarn install --immutable`) müsste beim nächsten PR nach main daran scheitern — main/develop-Lockfile-Stand evtl. divergiert. Trigger: nächster PR nach main ODER CI-Round. Action required: Lockfile-Format klären (regenerieren mit yarn 4.6.0 in einem kontrollierten Commit ODER packageManager-Pin auf yarn classic korrigieren) und CI-Run gegen develop erzwingen (workflow_dispatch).
- [ ] CB-15 LOW (entdeckt 2026-09-24): `test.yml` triggert nur bei push auf `main`, PRs und workflow_dispatch — develop-Pushes laufen OHNE CI. Die „CI autoritativ"-Annahme gilt für develop daher nur indirekt (erst beim PR). Trigger: nächste CI-Runde. Action: `develop` zu den push-Branches hinzufügen (oder bewusst dokumentieren).
