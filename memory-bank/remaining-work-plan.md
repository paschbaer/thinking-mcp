...
- [ ] CB-3 MEDIUM (Re-Evaluation des akzeptierten LOW aus L284b): Insight `server.ts` L94-99 UND clear-thought `server.ts` L97-102 erzeugen bei POST mit unbekannter/fehlender Session-Id Server+Transport, der ohne `initialize` NIE registriert wird → Reaper-blind. Default loopback okay, aber Compose-Publishing (0.0.0.0, keine Auth) macht daraus Resource-Churn pro Request (DoS-Amplifier). Trigger: nächste HTTP-/Deployment-Wartung insight+clear-thought. Action required (Hardening): Early-Reject für non-initialize ohne Header (body.method-Check) oder sofortige Registrierung mit pending-State.
...
- [ ] CB-11 LOW Insight `evidence/store.ts` `pathFor()`: kein Hash-Format-Check vor `join()` — Traversal-String erreichbar; Read-Hash-Verifikation macht Ausnutzung unwahrscheinlich (Existenz-Orakel bleibt). Trigger: nächste store.ts-Wartung. Action: `^[a-f0-9]{64}$`-Validierung.
- [ ] CB-12 LOW Insight `searchFullText`: sanitierter MATCH kann Bare-Keyword (z.B. NOT) bleiben → möglicher FTS5-Syntaxfehler roh an Client (unverifiziert). Trigger: nächster FTS-Touch. Verifizieren + ggf. Quotieren.
